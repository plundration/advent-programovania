...
Potom si vytvorte v priečinku nový súbor s názvom `main.go`.
...
Program spustíte buď stlačením `F5` alebo príkazom `go run main.go` v termináli,
ktorý sa buď nachádza na spodku okna, alebo si ho otvoríte cez `Terminal` > `New Terminal`.
...
func main() {
	...
	for {
		...
		fmt.Print("Ráno vstávam, budím starú, \"stará vstávaj variť kávu!\", " +
